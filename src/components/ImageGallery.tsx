...
export function ImageGallery({ images, onClose, onUpload, onDelete, onSelect, selectMode = false }: ImageGalleryProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-military-800 border border-military-700 rounded-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-military-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-white">图片仓库</h2>
            <span className="text-sm text-military-400">{images.length} 张</span>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-military-700 rounded-lg text-military-400 hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-military-700">
          <label className="flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg cursor-pointer transition-colors">
            <Upload className="w-5 h-5" />
            <span>上传图片到仓库</span>
            <input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" />
          </label>
          {selectMode && (
            <p className="text-xs text-military-400 mt-2 text-center">
              点击图片可选择使用
            </p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {images.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-military-500">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {images.map((img) => (
                ...
                >
                  <div className="aspect-square bg-military-900">
                    <img
                      src={img.imageBase64}
                      alt={img.name}
